...
| D-28 | **`main` is not deploying to Vercel** | **OPEN — raised Phase 0.3I, and it outranks most of this register.** `origin/main` = `4cf7787`, working tree clean, branch correct. 13 minutes after the push, `GET /api/v1/onboarding/google-forms` still returns **405** (old POST-only handler) on **both** `athlytica-systems-engine.vercel.app` and `www.nairobihockey.com`; the retired build exports `GET` and returns **410**. CLAUDE.md budgets ~3 minutes. **This is not the `master` mistake** — the branch is right. Consequence: **every verified result from 0.3D–0.3H describes code production is not running.** The database has M4; the deployed application does not call it. Could not read Vercel from here — MCP returns **403** for this project and `gh` is not installed. Check the dashboard for a failed build, a disconnected Git integration, or a paused project. |
