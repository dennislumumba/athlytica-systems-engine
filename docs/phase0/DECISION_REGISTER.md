...
| D-43 | **⛔ M6 BLOCKER — retry-on-23505 is not implemented at any caller** | **OPEN — found 2026-08-15 during M6 pre-application verification. M6 was NOT applied.** M6's own contract states the probe is advisory and *"callers must retry on 23505"*. **No caller does.** There are three, and each fails differently. **(1) `lib/services/bigice-onboarding.ts:217`** recognises `23505` but maps it to `reviewRequired: true` with the message *"an athlete named X already exists with no household contact to distinguish them"* — then returns. Under M6 a `23505` on `bigice_athlete` can mean **either** a duplicate `name_key` **or** a collided `biif_code`, so a code collision would be reported to a paying family as a duplicate-name problem, and no retry happens. **(2) `app/api/v1/workspaces/nrhl/onboard-paid-athlete/route.ts:188`** has **no `23505` handling at all**, and its `upsert(..., { onConflict: "display_name" })` **does not absorb an `athlete_code` PK collision** — the conflict target is the wrong column, so a collision surfaces as `upsertError` → **500 to a caller who has already paid**. **(3) `app/api/v1/leagues/nrhl/ingest/route.ts:146` is the serious one and it is structural, not just missing error handling.** It mints codes for every athlete **in a loop** and performs a **single batch upsert afterwards**, so the issuer's `not exists` probe **cannot see the codes minted earlier in the same batch**. Two athletes in one import can therefore receive the same code. Measured: **P(at least one intra-batch collision) = 1.4% at k=50, 5.4% at k=100, 21.5% at k=209, 75% at k=500** — and **k=209 is the legacy corpus**, i.e. the single largest planned use of this issuer (Phase 0.5). A caller-level retry does not cleanly fix this; the collision must be prevented in-batch (track minted codes and redraw) or the mint moved inside the insert. **The old monotonic sequence could not collide intra-batch, so this risk is created by Option C and did not exist before.** **Not fixed — the brief forbids improvising.** M6 is otherwise verified: see D-33. |
| D-44 | **Baseline omits the `scalable_id_sequence` seed row** | **OPEN — minor, found 2026-08-15; D-40 residue, not an M6 blocker.** `athlytica_core.scalable_id_sequence` is a counter implemented as a **table whose single row is its state**. Production holds 1 row (`id=1, current_value=504`); the reconstruction holds **0**, because `00000000000000_baseline_pre_migrations.sql` is schema-only by design. Consequence: in the local reconstruction `athlytica_core.generate_scalable_athlete_code()` would raise *"row id=1 is missing"*. It does not affect M6 — after M6 neither venture issuer reads the table — and it does not affect production. Fix is one guarded `insert ... on conflict do nothing` in the baseline, deliberately not made here to keep this turn scoped. |
...
| D-46 | **⚠ `FORCE ROW LEVEL SECURITY` is bypassed by `postgres` and `service_role` — do not describe it as universal RLS protection** | **OPEN — recorded 2026-08-15 from the D-35 matrix, and it re-grades D-01c.** Measured on production: `service_role` and `postgres` both carry **`rolbypassrls = true`**; `authenticated` does not. **The `BYPASSRLS` role attribute takes precedence over `FORCE`.** `FORCE` makes the table *owner* subject to policies; it does nothing to a role that bypasses RLS by attribute. Every table here is owned by `postgres`, and every `SECURITY DEFINER` function is owned by `postgres`, so **all of them bypass RLS with or without FORCE**. Two consequences. **(1) The feared breakage cannot happen:** D-35 case 9 confirmed `settle_payment_transaction` under FORCE on `registrations` returns `outcome=SETTLED` and sets `payment_status=PAYMENT_SETTLED`, because it runs as `postgres`. **(2) D-01c is much weaker than recorded.** "Only 2 of 64 tables have FORCE RLS" was written as a gap; adding FORCE would change nothing for any current caller, because the owner bypasses by attribute anyway. FORCE only becomes meaningful if a table is ever owned by a non-`BYPASSRLS` role. **RLS in this project governs `authenticated` and nothing else.** Any statement that RLS or FORCE protects data "from everyone" is false, and the security posture for service paths rests on grants, on the API gate, and on `SECURITY DEFINER` discipline — not on RLS. |
| D-47 | **Production default grants are not reproduced by the migration chain** | **OPEN — reconstruction-fidelity finding, 2026-08-15.** The D-40 baseline plus the 38 migrations reproduce production's tables, columns, constraints, indexes, functions, triggers, views and policies exactly (verified object-for-object) — **but not its GRANTs on migration-created tables.** Those come from Supabase's hosted default privileges, which no migration expresses and the local stack applies differently. Measured on `public.athletes`: production has `authenticated → SELECT` and `service_role → full DML`; the fresh reconstruction had **`authenticated → nothing`** and `service_role → REFERENCES, TRIGGER, TRUNCATE only`. **Consequence: D-35's `authenticated` cases would have tested a permission wall rather than RLS.** They were made faithful by replaying **production's exact grant statements** into the local database — widening nothing beyond what production already holds — and that patch is recorded here rather than hidden, because it is not derived from the migration chain and will be needed again on every fresh reconstruction. **Fix options (not chosen):** capture the grants into the baseline, or add an explicit grants migration. Either makes the reconstruction self-sufficient; both need a decision about whether the repository should own privileges that Supabase currently owns. Related but distinct from D-44 (missing `scalable_id_sequence` seed row). |
