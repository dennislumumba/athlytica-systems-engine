...
| **D-33** — identifier scheme | ⛔ **STOPPED for sign-off. B and B′ both withdrawn.** The rigorous pass found **two identifier planes**: `passport_id` (`text`, UNIQUE, 0 issued, Convex) and `athlete_code` (`character(9)`, PK, public verify). **R4 is venture-plane only**, so a 13-char passport format never belonged there. **Recommendation: Option C** — keep `ATH-NNNNN`, issue at random from `ATH-10000`–`ATH-99999`. Zero schema/regex/endpoint/integration change, 15× legacy margin, safe against the unmodified normaliser, R15 becomes impossible, M1's lock disappears. Cost: **90,000 hard ceiling**. |
