...
| **D-40** — migration history | 🟡 **BASELINE BUILT AND VERIFIED.** `00000000000000_baseline_pre_migrations.sql` supplies the 31 orphan tables; verified object-for-object against production (31 tables / 273 columns / 120 constraints / 23 indexes / 5 fns / 3 triggers / 3 views / 0 `anon` grants). **Local replay now reaches 10 files deep** — the `public.athlete` blocker is gone. Stops at **D-41**. Ledger rewrite still unresolved; drift arithmetic stays withdrawn. |
| **D-41** — invalid historical migration | ⛔ **NEW.** `20260713_…_rls.sql` uses `CREATE POLICY IF NOT EXISTS` ×5 — **never valid in any PostgreSQL version**, so the file has never been executable, and its named policies do not exist in production. Second proven instance of the D-31 pattern. Not modified (it is evidence). **Blocks completing the replay, which blocks M6.** |
