...
| **D-40** — migration history | ⛔ **RECONCILED, NOT FIXED.** The migration directory **cannot rebuild the database**: 31 of 67 tables have no creating migration, and the first file FK-references `public.athlete`, which nothing creates — hence the `npx supabase start` failure. **The ledger was also rewritten between 08-13 and 08-15** and now describes the repository rather than the database. Fix (a read, not a write): generate a baseline from `supabase db dump`. **Blocks applying M6 and blocks D-35 locally.** |
