...
| **D-31** — inventory allocation trigger | ✅ **CLOSED — superseded.** Valid SQL but a **proven no-op** here: `commercial_inventory` has 0 rows, `organization_source` exists nowhere, and `public.athlete` has neither size column. Behavioural test in the reconstruction: athlete insert wrote **0** waitlist rows. Moved byte-identical; no production object created, dropped or altered. Its caller's workflow is **D-42**, a separate product decision. |
