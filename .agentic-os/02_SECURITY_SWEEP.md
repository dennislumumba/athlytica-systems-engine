...
| `app/api/v1/onboarding/google-forms/route.ts` | ✅ `TENANT-EXEMPT` justification wrapper in file header (2026-07-12) | EXEMPT — pre-tenant identity creation; HMAC-SHA256 machine auth precedes all parsing/DB work; no tenant-scoped read path. Exemption voids if any standing condition in the header is violated. |
